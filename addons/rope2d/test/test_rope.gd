...
func _process(delta: float) -> void:
	...
	if Input.is_key_pressed(KEY_DOWN) and gate <= 0:
		...
		print("Growing rope")
		grow_rope.spool(1)
	if Input.is_key_pressed(KEY_LEFT) and grow_rope and not rope_drawer:
		rope_drawer = RopeDrawSimpleLine.new(grow_rope)
		add_child(rope_drawer)
	if Input.is_key_pressed(KEY_RIGHT) and grow_rope and rope_drawer:
		rope_drawer.queue_free()
		rope_drawer = null
	if Input.is_key_pressed(KEY_ENTER) and extend_rope and gate <= 0:
		gate = 0.2
